{ config, pkgs, lib, ... }:

{
  imports = [ <nixpkgs/nixos/modules/virtualisation/google-compute-image.nix> ];

  # Sudo
  security.sudo.wheelNeedsPassword = false;
  # SSH
  services.openssh.enable = true;
  services.openssh.permitRootLogin = lib.mkOverride 10 "no";
  services.openssh.challengeResponseAuthentication = false;
  services.openssh.passwordAuthentication = false;
  # Users
  users.mutableUsers = false;
  users.users.packer = {
    isNormalUser = true;
    extraGroups = [ "wheel" ];
    openssh.authorizedKeys.keys = [
      "ssh-rsa "
    ];
  };
  # Services
  services.gitea.enable = true;
  services.gitea.appName = "4Linux - Gitea";
  services.gitea.database.createDatabase = false;
  services.gitea.database.host = "10.128.15.227";
  services.gitea.database.password = "4linux@gitea";
  services.gitea.database.port = 5432;
  services.gitea.database.type = "postgres";
  services.gitea.repositoryRoot = "/mnt/data/repositories";
  services.gitea.stateDir = "/mnt/data";
  # FS
  fileSystems."/mnt/data" = {
    device = "/dev/disk/by-id/google-gitea";
    fsType = "xfs";
    autoFormat = true;
    options = [ "noatime" "nodiratime" "discard" ];
  };
  boot.growPartition = true;
}

